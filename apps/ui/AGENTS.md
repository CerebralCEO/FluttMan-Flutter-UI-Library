# Particle Component Development Guide for @flutman/ui
...
**Important:** Always use the `@/registry/default/ui/` path, never `@flutman/ui/components/` in particles.
...
**Where it exists:** only the wrappers listed in `apps/ui/skills/flutman/references/portal-props.md`. Do not add `portalProps` to registry helpers that are not on that list (their portal stays internal).
...
## 10. Migration from Origin (shadcn/Radix) to flutman (Base UI)
...
3. **Compare the primitives:**
   ...
   - flutman uses Base UI primitives
...
5. **Understand the differences:**
   ...
   - Size differences (flutman is more compact)
...
| Component | shadcn/ui Size | flutman ui Size |
|-----------|----------------|--------------|
...
**flutman ui is more compact by default, so use larger sizes to match shadcn/ui appearance.**
...
4. **Look at existing flutman particles** - see if similar patterns exist
...
// flutman might handle this automatically, or need:
...
```tsx
{
  ...
  registryDependencies: ["@flutman/button", "@flutman/spinner"],
...
- `registryDependencies`: Array of `@flutman/{component}` package names
...
- [ ] Included all `@flutman/*` dependencies in `registryDependencies`
...
4. **Compare primitives:**
   ...
   - Check what Base UI primitive flutman uses
   - Understand why classes were overridden in origin
   - Determine if the same override is needed in flutman
...
6. **Test and verify:**
   ...
   - Ensure it follows flutman patterns
...
- Verify size mapping (shadcn → flutman)
...
- Verify `registryDependencies` use `@flutman/*` format
