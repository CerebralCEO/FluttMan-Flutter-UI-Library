...
description: A practical guide for migrating from Radix-based libraries to flutman ui components.
---

This guide is for developers who already have applications built on Radix-based libraries — frameworks that wrap Radix primitives — and **want to adopt flutman ui**.

**flutman ui** is built on top of **[Base UI](https://base-ui.com/)**, using a custom abstraction that stays intentionally close to its mental model. The component API exposed by flutman ui is intentionally similar to shadcn/ui, which is why this guide also highlights the main differences between shadcn/ui and flutman ui.

## Overview

**flutman ui** uses **Base UI** internally, not Radix. This means the architecture and API patterns are different by design. However, we've worked to make the migration path as clear as possible by:
...
**Tip:** This migration guide is structured to be LLM-friendly. You can provide this page to AI coding assistants (like Claude, ChatGPT, etc.) to help automate the conversion of your Radix components to flutman ui.
...
Not every component uses this pattern. Apply it where the flutman component supports `render`.

<span data-lib="base-ui">
```tsx title="flutman ui"
// [!code word:render]
...
So, if you want to preserve the original shadcn/ui badge size, you should use the `lg` size in flutman ui.
...
flutman ui button sizes are more compact compared to shadcn/ui, making them better suited for dense applications. We also introduce new sizes (`xs`, `xl`, `icon-sm`, `icon-lg`) for more granular control:

| Size      | Height (shadcn/ui) | Height (flutman ui) |
| --------- | ------------------ | ---------------- |
| `xs`      | -                  | 24px             |
| `sm`      | 32px               | 28px             |
| `default` | 36px               | 32px             |
...
So, for example, if you were using the `default` size in shadcn/ui and you want to preserve the original height, you should use the `lg` size in flutman ui.
...
So, if you want to preserve the original shadcn/ui input height (36px), you should use the `lg` size in flutman ui.
...
flutman does **not** use the community [`input-otp`](https://github.com/guilhermerodz/input-otp) package. OTP entry is implemented with [Base UI OTP Field](https://base-ui.com/react/components/otp-field) (`OTPFieldPreview`) via the `@flutman/otp-field` registry item (`components/ui/otp-field.tsx`).

**Quick Checklist:**
- Remove the `input-otp` dependency from your app; rely on `@base-ui/react` through flutman
...
| Component     | Radix UI Prop                                  | flutman ui / Base UI Prop                      |
| ------------- | ---------------------------------------------- | ------------------------------------------- |
...
flutman ui select sizes are more compact compared to shadcn/ui, making them better suited for dense applications:
...
`alignItemWithTrigger` controls whether the popup aligns the selected item with the trigger text. In flutman ui this prop is available on `SelectPopup` (legacy alias: `SelectContent`).
...
**Quick Checklist:**
- flutman ui `Slider` uses Base UI's multiple value approach
...
flutman ui toggle group sizes are more compact compared to shadcn/ui, making them better suited for dense applications:
