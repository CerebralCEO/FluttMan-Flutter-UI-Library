...
Components like Dialog, AlertDialog, Sheet, and Empty use the `font-heading` class for titles. The flutman style preset defines `--font-sans`, `--font-heading`, and `--font-mono` with system font fallbacks. To use custom fonts, define these variables in your layout using `next/font`:
