...
This guide provides the essentials for adding **flutman ui** components to your React application.
...
For new projects, use the **flutman style** preset to initialize everything in one command:

```bash
npx shadcn@latest init @flutman/style
```
...
```bash
npx shadcn@latest add @flutman/ui
```
...
```bash
npx shadcn@latest add @flutman/style
```

Or add only the color tokens:

```bash
npx shadcn@latest add @flutman/ui @flutman/colors-neutral
```
...
Components are styled with the design token system we use at **ZeroFounder**, which is defined by CSS variables and implemented with Tailwind CSS.
...
flutman components use three CSS custom properties for typography:
...
When you use `@flutman/style`, `@flutman/fonts` is installed automatically — Inter for `--font-sans` and `--font-heading`, Geist Mono for `--font-mono` — all wired in your `layout.tsx`.
...
**Note:** Next.js starters default to variable names like `--font-geist-sans`, which don't match flutman's expected `--font-sans`. If fonts appear broken after setup, check that the variable names match.
...
```tsx
import { Slider, SliderValue, SliderPrimitive } from "@flutman/ui/components/slider";
```

## Base UI Re-exports

When you only need `useRender`, `mergeProps`, `CSPProvider`, or `DirectionProvider`, import them from `@flutman/ui/base-ui/*` instead of adding `@base-ui/react` as a direct dependency:

- `@flutman/ui/base-ui/use-render`
- `@flutman/ui/base-ui/merge-props`
- `@flutman/ui/base-ui/csp-provider`
- `@flutman/ui/base-ui/direction-provider`

This is especially useful in **monorepos**: apps and packages that already depend on `@flutman/ui` can use these utilities without adding `@base-ui/react` as another dependency. You avoid duplicate installs, keep versions in sync, and simplify your dependency tree—`@base-ui/react` lives in one place (`@flutman/ui`), and everything else imports through it.
...
- **[Agent Skills](/ui/docs/skills)** — Install flutman knowledge directly into your AI assistant with `npx skills add flutman/flutman`. Covers all 53 primitives, styling conventions, migration patterns, and particle examples.
