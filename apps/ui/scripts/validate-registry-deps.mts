...
// Map component name to @flutman package name
function getRegistryPackageName(componentName: string): string {
  return `@flutman/${componentName}`;
}
...
function parseImports(content: string): {
  ...
  while (match !== null) {
    ...
    // Check if it's already a @flutman import
    if (importPath.startsWith("@flutman/")) {
      // Skip @flutman/utils as it's a common utility that shouldn't be treated as a dependency
      if (importPath !== "@flutman/utils") {
...
function extractRegistryItemDeps(itemContent: string): {
  ...
  if (regDepsMatch) {
    regDepsMatch[1].split(",").forEach((item) => {
      ...
      // Skip @flutman/utils as it's a common utility that shouldn't be validated
      if (cleaned && cleaned !== "@flutman/utils") {
...
async function isDependencyCoveredByRegistry(
  ...
  for (const regDep of registryDeps) {
    // Extract component name from @flutman/component-name
    const componentMatch = regDep.match(/@flutman\/(.+)/);
