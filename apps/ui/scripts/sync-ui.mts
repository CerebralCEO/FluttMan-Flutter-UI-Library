...
function rewriteImports(code: string): string {
  ...
  // "@/lib/*"        → "@flutman/ui/lib/*"
  // "@/hooks/*"      → "@flutman/ui/hooks/*"
  // "@/registry/default/ui/*" → "@flutman/ui/components/*"
  // "@/registry/default/hooks/*" → "@flutman/ui/hooks/*"
  // "@/registry/default/lib/*" → "@flutman/ui/lib/*"
  result = result.replace(/(["'])@\/lib\//g, "$1@flutman/ui/lib/");
  result = result.replace(/(["'])@\/hooks\//g, "$1@flutman/ui/hooks/");
  result = result.replace(
    /(["'])@\/registry\/default\/ui\//g,
    "$1@flutman/ui/components/",
  );
  result = result.replace(
